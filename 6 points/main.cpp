...
int main(int argc, char *argv[]) {
    ...
    if (result1 < 0) {
        ...
        // Waiting for child process 2 to finish
        int status;
        waitpid(result1, &status, 0);

        size = read(fd2[0], str_buf, buf_size);
        if (size < 0) {
            printf("Process1: Can\'t read string from pipe\n");
            exit(-1);
        }

        int outputFileDescriptor = open(outputPath, O_WRONLY | O_CREAT | O_TRUNC);
        if (outputFileDescriptor == -1) {
            printf("Process1: Can't open file on specified output path!\n");
            exit(-1);
        }
        ssize_t bytes_write = write(outputFileDescriptor, str_buf, size);
        if (bytes_write == -1) {
