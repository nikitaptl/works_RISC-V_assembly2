...
int main(int argc, char *argv[]) {
  ...
  int clientSock, observerSock;
  struct sockaddr_in servAddr, clientAddr, observerAddr;
  socklen_t clientAddrLen, observerAddrLen;
  ...
  if (listen(servSock, NUM_PROGRAMMERS + 1) < 0) {
  ...
  AcceptConnection(servSock, observerSock, observerAddr, observerAddrLen, message);
  ...
  while (1) {
    if (free_programmers != 0) {
      ...
      if (is_new_task) {
        ...
        sendto(observerSock, message, sizeof(message), 0, (struct sockaddr *) &observerAddr, observerAddrLen);
        server_message(message);
