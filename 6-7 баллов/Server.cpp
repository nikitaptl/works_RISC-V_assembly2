...
int main(int argc, char *argv[]) {
  ...
  socklen_t clientAddrLen = sizeof(clientAddr), observerAddrLen = sizeof(observerAddr);
  ...
  server_message("All programmers connected. Waiting for observer to connect...");
  // Принятие соединения наблюдателя
  Task observer_init_task;
  ssize_t recv_len = recvfrom(servSock, &observer_init_task, sizeof(Task), 0, (struct sockaddr *)&observerAddr, &observerAddrLen);
  if (recv_len < 0) {
    DieWithError("recvfrom() failed");
  }
  ...
  while (1) {
    if (free_programmers != 0) {
      ...
      if (is_new_task) {
        ...
        if (sendto(servSock, &new_task, sizeof(Task), 0, (struct sockaddr *)&programmers[id].echoServAddr, sizeof(programmers[id].echoServAddr)) != sizeof(Task)) {
        ...
        sendto(servSock, message, sizeof(message), 0, (struct sockaddr *)&observerAddr, observerAddrLen);
        server_message(message);
