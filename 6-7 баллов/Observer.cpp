#include "common.h"

int serverSocket;
char message[256];

void DieWithError(const char *message) {
  error_message(message);
  perror("");
  exit(1);
}

void sig_handler(int sig) {
  if (sig != SIGINT && sig != SIGQUIT && sig != SIGHUP) {
    return;
  }
  observer_message("Received a stop signal. Bye!");
  close(serverSocket);
  exit(10);
}

int main(int argc, char *argv[]) {
  signal(SIGINT, sig_handler);
  signal(SIGQUIT, sig_handler);
  signal(SIGHUP, sig_handler);

  struct sockaddr_in serverAddress;
  unsigned short serverPort;
  char *serverIP;
  socklen_t serverAddressLen = sizeof(serverAddress);

  if (argc < 2 || argc > 3) {
    DieWithError("Wrong number of arguments. Write Server IP and Port(optional)");
  }
  serverIP = argv[1];
  serverPort = (argc == 3) ? atoi(argv[2]) : 60000;

  // Создаем UDP сокет
  if ((serverSocket = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
    DieWithError("socket() failed");
  }

  memset(&serverAddress, 0, sizeof(serverAddress));
  serverAddress.sin_family = AF_INET;
  serverAddress.sin_addr.s_addr = inet_addr(serverIP);
  serverAddress.sin_port = htons(serverPort);

  sprintf(message, "Connected to server, ip = %s, port = %d", inet_ntoa(serverAddress.sin_addr), serverPort);
  observer_message(message);

  while (1) {
    int result = recvfrom(serverSocket, message, sizeof(message) - 1, 0, (struct sockaddr *)&serverAddress, &serverAddressLen);
    if (result < 0) {
      DieWithError("Can not read message from server");
    } else if (result == 0) {
      sig_handler(SIGINT);
    } else {
      message[result] = '\0'; // Завершаем строку нулевым символом
      observer_message(message);
    }
  }
}
