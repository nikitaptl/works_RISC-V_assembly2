...
pthread_cond_t condition; // Условная переменная
...
void *encryptString(void *param) {
    ...
    // Условная переменная - пока индикатор isBlocked true, нет доступа для чтения
    // Ждём пробуждения от другого потока
    while (isBlocked) {
    ...
    // Ставим индикатор isBlocked false и пробуждаем рандомный поток из оставшихся
    isBlocked = false;
